...
# Simple matrix for quick testing
# TEST_CONFIGURATIONS = list(product(
#     ["page_first"],  # layouts
#     [64],              # page_sizes
#     ["kernel"],       # io_backends
...
def cleanup_gpu_memory():
    """Silently clean up GPU memory for tests (minimal)."""
...
def print_memory_details():
    """Print detailed GPU memory info (debug only)."""
    if torch.cuda.is_available():
...
def create_test_sequences(num_sequences: int, seq_length: int, page_size: int) -> List[List[int]]:
    """Create test token sequences for testing"""
    sequences = []
    for i in range(num_sequences):
        # Create sequences with some overlap for testing prefix matching
        base_start = i * (seq_length // 2)  # Overlapping sequences
        # Align to page size
...
def release_hicache_resources(hicache, _unused_allocator, token_to_kv_pool):
    _ = _unused_allocator
    """Minimal release: stop threads, drop KV buffers, sync."""
    if hicache is not None and hasattr(hicache, "reset"):
        try:
            hicache.reset()
        except Exception:
            pass
    if token_to_kv_pool is None and hicache is not None and hasattr(hicache, "kv_cache"):
        token_to_kv_pool = hicache.kv_cache
...
class HiCacheTestContext:
    """Context manager to manage HiCache lifecycle and GPU cleanup in tests."""
    ...
    def __exit__(self, exc_type, exc, tb):
        ...
        try:
            if self.hicache is not None and hasattr(self.hicache, "reset"):
                self.hicache.reset()
            # Release GPU/CPU buffers explicitly
            release_hicache_resources(
                self.hicache, self.token_to_kv_pool_allocator, self.token_to_kv_pool
            )
        finally:
...
class TestHiCacheSimpleIntegration:
    """Simple HiRadixCache Integration Tests"""

    def _run_configuration_test(self, distributed_group, layout: str, page_size: int, io_backend: str, attention_type: str):
        """Test a specific configuration combination"""
        _ = distributed_group  # distributed_group is available via fixture
        ...
        with HiCacheTestContext(config) as ctx:
            ...
            # Test 1: Insert sequences and verify they are cached
            start_time = time.monotonic()
            inserted_tokens = []
            for i, sequence in enumerate(test_sequences):
                # Allocate KV cache for the sequence
                ...
                # Insert into HiRadixCache
                prev_total = hicache.total_size()
                _ = hicache.insert(sequence, kv_indices)
                new_total = hicache.total_size()
                inserted_tokens.append(max(0, new_total - prev_total))

                # Check if insertion was successful (new tokens inserted should be non-negative)
                assert inserted_tokens[-1] >= 0, f"Insert failed for sequence {i}"
            ...
            start_time = time.monotonic()
            for i, sequence in enumerate(test_sequences):
                # Test full sequence match
                match_result = hicache.match_prefix(sequence)
                assert match_result.device_indices is not None, f"Match failed for sequence {i}"

                # Test partial sequence match
                if len(sequence) > config.page_size:
                    partial_seq = sequence[: len(sequence) // 2]
                    partial_result = hicache.match_prefix(partial_seq)
                    assert partial_result.device_indices is not None, f"Partial match failed for sequence {i}"
                    # Should match at least some tokens
                    assert len(partial_result.device_indices) > 0, f"No tokens matched for partial sequence {i}"
            ...
            # sleep a bit to wait for prefetch to complete
            time.sleep(1)

            for i, sequence in enumerate(test_sequences):
                complted = hicache.check_prefetch_progress(f"req_{i}")
                assert complted, f"Prefetch did not complete for sequence {i}"
            ...
            # Basic assertions
            assert insert_time > 0, "Insert operations should take some time"
            assert match_time >= 0, "Match operations should complete"
            assert sum(inserted_tokens) > 0, "Should have inserted some tokens"

            # Test cache statistics
            total_size = hicache.total_size()
            evictable_size = hicache.evictable_size()
            assert total_size >= 0, "Total cache size should be non-negative"
            assert evictable_size >= 0, "Evictable size should be non-negative"
    ...
    def test_allocation_failure_recovery(self, distributed_group):
        ...
        # Use smaller memory pool to trigger allocation failures more easily
        ...
        with HiCacheTestContext(config) as ctx:
            ...
            # Phase 1: Fill up memory pool by allocating AND inserting into cache
            for i in range(12):  # Try to fill up 12*1024 tokens (max capacity)
            ...
            # Phase 2: Try to allocate one more sequence - should fail due to memory exhaustion
    ...
    def test_storage_integration(self, distributed_group):
        ...
        config.hicache_storage_backend = "test_storage"
        hicache = None
        ...
        try:
            hicache, req_to_token_pool, token_to_kv_pool_allocator, token_to_kv_pool = create_hicache_system(config)
            ...
            # Test that storage operations don't crash
            # The actual storage validation would require more complex setup
            ...
            # Release large KV buffers and related resources
            release_hicache_resources(hicache, token_to_kv_pool_allocator, token_to_kv_pool)

            # Drop references
            del hicache, req_to_token_pool, token_to_kv_pool_allocator, token_to_kv_pool

            # Finally, cleanup GPU memory
            cleanup_gpu_memory()
    ...
    def test_all_configurations(self, distributed_group, layout, page_size, io_backend, attention_type):
        ...
        self._run_configuration_test(distributed_group, layout, page_size, io_backend, attention_type)
